...
Core routes: `POST /auth/login`, `POST /auth/refresh`, `POST /devices/register`, `GET /me`, `GET /app-config`, lead CRUD/timeline, call/call-event creation, dispositions, notes, follow-ups, queue, today stats, `POST /sync/batch`, and `GET /sync/changes?cursor=`.
